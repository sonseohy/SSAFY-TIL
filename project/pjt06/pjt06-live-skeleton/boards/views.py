...
def index(request):
    ...
    # - 프론트 측 (django에선 template)에서 404 error를 처리해줬다면 사용
...
@require_http_methods(["GET", "POST"])
def create(request):
    # @login_required를 안쓰고 밑의 코드로 로그인하지 않은 사용자는 접근하지 못하도록 할 수 있음
    # if request.user.is_authenticated:
    #     return redirect("boards:index")
    # @login_required의 장점(차이) : 보내는 url에 차이가 있다. (리디렉션하는 URL이 다름)
