...
def dijkstra(start):
    ...
    heapq.heappush(pq, (0, start))  # 누적값 0이 앞에 나와야 함
    ...
    while pq:
        ...
        # 예제그림으로 보면 c 위치 가중치 3, 4로 도착가능 [참고]
        ...
        for next in graph[now]:
            ...
            new_cost = dist + cost  # 누적값 계산 (현재까지의 누적 값 + 다음 노드 가중치)

            # 다음 노드를 가는 데 더 많은 비용이 드는 경우 (누적값이 더 큰 경우 갈 필요 없음)
...
# 즉, 다익스트라 한 번이면, 하나의 정점에서 다른 저점들까지의 최단거리들을 모두 구할 수 있다.