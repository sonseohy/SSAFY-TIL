...
def shoot(level, remains, now_arr):
    ...
    for col in range(W):
        ...
        # 방법 2. (방법 1은 복구 하는 시간이 너무 오래걸리므로)
        # 1. col 위치에 쏘기 전 상태를 복사
        ...
        row = -1 # 없을수도 있으므로 없다고 가정
        for r in range(H):
            if copy_arr[r][col]:    # r 위치에 벽돌이 있다면
                row = r # 가장 위
                break
        if row == -1:   # 벽도이 없는 열이면 다음 열로 넘어가자
            continue

        # 2. 연쇄적으로 벽돌이 깨짐
        # 행, 열, 숫자(파워)
        li = [(row, col, copy_arr[row][col])]  # 깨져야 할 벽돌들을 저장
...
for test_case in range(1, T+1):
    ...
    # 1. 최소로 남은 벽돌
    # - 몇 개 남았을까 계산해야 한다 -> 2차원 리스트를 순회하면서 매번 계산하면 너무 느리고 비효율적
    # - 현재 남은 벽돌 수를 통해 해결됨
