...
const submitReview = () => {
  if (title.value && content.value) {
    ...
    console.log("제출된 리뷰:", review); // 콘솔로 출력해서 값 확인
    ...
    calendar.addEvent({
      ...
      extendedProps: {
        poster: review.poster_path, // 포스터 경로 저장
      }
    });
...
onMounted(() => {
  ...
  calendar = new FullCalendar.Calendar(calendarEI, {
    initialView: 'dayGridMonth', // 기본 달력 뷰 설정
    selectable: true, // 날짜 선택 가능
    events: reviews.value.map(review => ({
      title: review.title,  // 영화 제목
      start: review.date,   // 작성된 날짜
      description: `내용: ${review.content}, 별점: ${review.rating} ★`, // 설명 (내용과 별점 포함)
      extendedProps: {
        poster: review.poster_path, // 포스터 경로 (추가적인 정보)
      }
    })),
    eventContent: function (arg) {
      const poster = arg.event.extendedProps.poster;
      const posterUrl = poster ? `https://image.tmdb.org/t/p/w200${poster}` : null;
      ...
      return {
        html: `
          <div>
            ...
            ${posterUrl ? 
              `<img src="${posterUrl}" alt="Poster" style="width: 50px; height: 75px; object-fit: cover;" />` 
              : ''}
          </div>
        `
      };
    },
    // 날짜 클릭 시 발생하는 이벤트 핸들러
    dateClick: function (info) {
      selectedDate.value = info.dateStr;  // 클릭한 날짜로 selectedDate를 업데이트
